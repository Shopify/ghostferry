...
func (f *ShardedCopyFilter) BuildSelect(columns []string, table *ghostferry.TableSchema, lastPaginationKey, batchSize uint64) (sq.SelectBuilder, error) {
	...
	if joinThroughTable, exists := f.JoinedThroughTables[table.Name]; exists {
		...
		// shared (typically global data) between multiple sharding values, but having a n-1 relationship to the join table,
		// so that table rows cannot be inidividually identified and paginated with sub-queries like in the "joined table"
		// case, and must be done with a JOIN clause instead.
		//
		// Target query:
		//
		// SELECT * FROM x
		...
		fullQuotedPaginationKey := fmt.Sprintf("%s.%s", quotedTable, quotedPaginationKey)
		return sq.Select(columns...).
			From(quotedTable).
			Join(quotedJoinTableName + " ON " + joinThroughTable.JoinCondition).
			Where(sq.Eq{quotedShardingKey: f.ShardingValue}).
			Where(sq.Gt{fullQuotedPaginationKey: lastPaginationKey}).
			OrderBy(fullQuotedPaginationKey).
			Limit(batchSize), nil
	}
