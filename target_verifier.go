...
func (t *TargetVerifier) BinlogEventListener(evs []DMLEvent) error {
	for _, ev := range evs {
		...
		if annotation == "" || annotation != t.DB.Marginalia {
			...
			return fmt.Errorf("row data with paginationKey %s on `%s`.`%s` has been corrupted by a change directly performed in the target at binlog file: %s and position: %d", paginationKey, ev.Database(), ev.Table(), ev.BinlogPosition().Name, ev.BinlogPosition().Pos)
		}
	}
